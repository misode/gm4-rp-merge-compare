...
execute as @a[gamemode=!spectator, distance=..3] at @s anchored eyes positioned ^ ^ ^1.448 if block ~ ~ ~ #minecraft:cauldrons align xyz positioned ~0.5 ~0.5 ~0.5 if entity @e[type=marker, tag=gm4_current_zauber_cauldron, distance=..0.1] align xyz as @e[type=item, dx=0, dy=0, dz=0] run data merge entity @s {PickupDelay:18s}
