...
execute positioned ~-0.2 ~-0.2 ~-0.2 as @e[type=minecraft:armor_stand, tag=gm4_bas_no_arms, dx=0, limit=1] positioned ~-0.6 ~-0.6 ~-0.6 run tag @s[dx=0] add gm4_bas_selected

# continue searching if not inside armor_stand
execute unless entity @e[type=minecraft:armor_stand, tag=gm4_bas_no_arms, tag=gm4_bas_selected, limit=1] unless entity @s[distance=6..] positioned ^ ^ ^0.5 run function gm4_better_armour_stands:book/ray_arms
